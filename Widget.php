...
/**
 * 找回密码类
 * @package TleMultiFunction
 */
class TleMultiFunction_Widget extends Typecho_Widget{
    /**
     * 配置表
     *
     * @access private
    ...
    public function __construct($request, $response, $params = NULL)
    {
        ...
        $this->config = $this->options->plugin('TleMultiFunction');
    }
    ...
    public function doForgot()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $phpMailer->Host = @$get["host"];
            $phpMailer->Port = @$get["port"];
            $phpMailer->Username = @$get["username"];
            $phpMailer->Password = @$get["password"];
            $phpMailer->isHTML(true);

            if ('none' != @$get["secure"]) {
                $phpMailer->SMTPSecure = @$get["secure"];
            }

            $phpMailer->setFrom(@$get["username"], $this->options->title);
    ...
    public function forgotForm()
    {
        ...
        /** 提交按钮 */
        $submit = new Typecho_Widget_Helper_Form_Element_Submit('submit', NULL, _t('通过邮箱找回'));
